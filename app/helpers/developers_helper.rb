module DevelopersHelper
  ...
  def tool_name(employee)
    employee.employee_tools.includes([:tool]).map { |employee_tool| employee_tool.tool.name }.join(', ')
  end

  def programming_language_name(developer)
    developer.developer_programming_languages.includes([:programming_language]).map { |developer_programming_language| developer_programming_language.programming_language.name }.join(', ')
  end

  def certificate_name(developer)
    developer.certificate_employees.includes([:certificate]).map { |certificate_employee| certificate_employee.certificate.name }.join(', ')
  end

  def framework_name(developer)
    developer.developer_frameworks.includes([:framework]).map { |developer_framework| developer_framework.framework.name }.join(', ')
  end
end
