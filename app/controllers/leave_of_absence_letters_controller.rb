class LeaveOfAbsenceLettersController < ApplicationController
  ...
  def index
    @leave_of_absence_letters = if current_user.admin?
                                  LeaveOfAbsenceLetter.order(id: :asc)
                                else
                                  LeaveOfAbsenceLetter.where(employee_id: current_user.employee_id).order(start_date: :asc)
                                end
